...
NS_ASSUME_NONNULL_BEGIN
    ...
    #define UbiXAutorelease(__v) ([__v autorelease]);
    #define UbiXReturnAutoreleased UbiXAutorelease

    #define UbiXRetain(__v) ([__v retain]);
    #define UbiXReturnRetained UbiXRetain

    #define UbiXRelease(__v) ([__v release]);

    #define UbiXDispatchQueueRelease(__v) (dispatch_release(__v));
#else
    // -fobjc-arc
    #define UbiXAutorelease(__v)
    #define UbiXReturnAutoreleased(__v) (__v)

    #define UbiXRetain(__v)
    #define UbiXReturnRetained(__v) (__v)

    #define UbiXRelease(__v)

// If OS_OBJECT_USE_OBJC=1, then the dispatch objects will be treated like ObjC objects
// and will participate in ARC.
// See the section on "Dispatch Queues and Automatic Reference Counting" in "Grand Central Dispatch (GCD) Reference" for details. 
    #if OS_OBJECT_USE_OBJC
        #define UbiXDispatchQueueRelease(__v)
    #else
        #define UbiXDispatchQueueRelease(__v) (dispatch_release(__v));
    #endif
#endif
 ...
 */
typedef int(^UbiXExecuteStatementsCallbackBlock)(NSDictionary *resultsDictionary);

/**
 Enumeration used in checkpoint methods.
 */
typedef NS_ENUM(int, UbiXDBCheckpointMode) {
    UbiXDBCheckpointModePassive  = 0, // SQLITE_CHECKPOINT_PASSIVE,
    UbiXDBCheckpointModeFull     = 1, // SQLITE_CHECKPOINT_FULL,
    UbiXDBCheckpointModeRestart  = 2, // SQLITE_CHECKPOINT_RESTART,
    UbiXDBCheckpointModeTruncate = 3  // SQLITE_CHECKPOINT_TRUNCATE
};
...
/** A SQLite ([https://sqlite.org/](https://sqlite.org/)) Objective-C wrapper.
 ...
 The three main classes in UbiXDB are:

 - @c UbiXDatabase - Represents a single SQLite database.  Used for executing SQL statements.

 - @c UbiXResultSet - Represents the results of executing a query on an @c UbiXDatabase .

 - @c UbiXDatabaseQueue - If you want to perform queries and updates on multiple threads, you'll want to use this class.

 See also

 - @c UbiXDatabasePool - A pool of @c UbiXDatabase objects

 - @c UbiXStatement - A wrapper for @c sqlite_stmt

 External links

 - [UbiXDB on GitHub](https://github.com/ccgus/fmdb) including introductory documentation
 - [SQLite web site](https://sqlite.org/)
 - [UbiXDB mailing list](http://groups.google.com/group/fmdb)
 - [SQLite FAQ](https://sqlite.org/faq.html)

 @warning Do not instantiate a single @c UbiXDatabase  object and use it across multiple threads. Instead, use @c UbiXDatabaseQueue .

 */

@interface UbiXDatabase : NSObject
...
/** Create a @c UbiXDatabase  object.
 
 An @c UbiXDatabase  is created with a path to a SQLite database file.  This path can be one of these three:

 1. A file system path.  The file does not have to exist on disk.  If it does not exist, it is created for you.

 2. An zero-length string.  An empty database is created at a temporary location.  This database is deleted with the @c UbiXDatabase  connection is closed.

 3. @c nil .  An in-memory database is created.  This database will be destroyed with the @c UbiXDatabase  connection is closed.
...
UbiXDatabase *db = [UbiXDatabase databaseWithPath:fileURL.path];
@endcode
 ...
 @return @c UbiXDatabase  object if successful; @c nil  if failure.
...
/** Create a @c UbiXDatabase  object.
 ...
 2. @c nil .  An in-memory database is created.  This database will be destroyed with the @c UbiXDatabase  connection is closed.
...
NSURL *folder  = [[NSFileManager defaultManager] URLForDirectory:NSApplicationSupportDirectory inDomain:NSUserDomainMask appropriateForURL:nil create:true error:&error];
NSURL *fileURL = [folder URLByAppendingPathComponent:@"test.db"];
UbiXDatabase *db = [UbiXDatabase databaseWithURL:fileURL];
@endcode
...
/** Initialize a @c UbiXDatabase  object.
 
 An @c UbiXDatabase  is created with a path to a SQLite database file.  This path can be one of these three:

 1. A file system path.  The file does not have to exist on disk.  If it does not exist, it is created for you.

 2. A zero-length string.  An empty database is created at a temporary location.  This database is deleted with the @c UbiXDatabase  connection is closed.
 ...
 NSURL *folder  = [[NSFileManager defaultManager] URLForDirectory:NSApplicationSupportDirectory inDomain:NSUserDomainMask appropriateForURL:nil create:true error:&error];
 NSURL *fileURL = [folder URLByAppendingPathComponent:@"test.db"];
 UbiXDatabase *db = [[UbiXDatabase alloc] initWithPath:fileURL.path];
 @endcode
...
/** Initialize a @c UbiXDatabase  object.
 ...
 An @c UbiXDatabase  is created with a local file URL to a SQLite database file.  This path can be one of these three:
 ...
 NSURL *folder  = [[NSFileManager defaultManager] URLForDirectory:NSApplicationSupportDirectory inDomain:NSUserDomainMask appropriateForURL:nil create:true error:&error];
 NSURL *fileURL = [folder URLByAppendingPathComponent:@"test.db"];
 UbiXDatabase *db = [[UbiXDatabase alloc] initWithURL:fileURL];
 @endcode
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(__attribute__((noescape)) UbiXExecuteStatementsCallbackBlock _Nullable)block;
...
/** Execute select statement
 ...
 Executing queries returns an @c UbiXResultSet  object if successful, and @c nil  upon failure.  Like executing updates, there is a variant that accepts an `NSError **` parameter.  Otherwise you should use the @c lastErrorMessage  and @c lastErrorMessage  methods to determine why a query failed.
 
 In order to iterate through the results of your query, you use a `while()` loop.  You also need to "step" (via `<[UbiXResultSet next]>`) from one record to the other.
 ...
 @return A @c UbiXResultSet  for the result set upon success; @c nil  upon failure. If failed, you can call @c lastError , @c lastErrorCode , or @c lastErrorMessage  for diagnostic information regarding the failure.
 
 @see UbiXResultSet
 @see [`UbiXResultSet next`](<[UbiXResultSet next]>)
 @see [`sqlite3_bind`](https://sqlite.org/c3ref/bind_blob.html)
...
- (UbiXResultSet * _Nullable)executeQuery:(NSString*)sql, ...;
...
- (UbiXResultSet * _Nullable)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
- (UbiXResultSet * _Nullable)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments;
...
/** Execute select statement
 ...
 `func executeQuery(sql: String, values: [Any]?) throws  -> UbiXResultSet!`
...
- (UbiXResultSet * _Nullable)executeQuery:(NSString *)sql values:(NSArray * _Nullable)values error:(NSError * _Nullable __autoreleasing *)error;
...
- (UbiXResultSet * _Nullable)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary * _Nullable)arguments;


// Documentation forthcoming.
- (UbiXResultSet * _Nullable)executeQuery:(NSString *)sql withVAList:(va_list)args;

/// Prepare SQL statement.
///
/// @param sql SQL statement to prepare, generally with `?` placeholders.

- (UbiXResultSet *)prepare:(NSString *)sql;
...
/** Begin a transaction
 ...
 @warning    Unlike SQLite's `BEGIN TRANSACTION`, this method currently performs
             ...
             is likely to change in future versions of UbiXDB, whereby this method
 ...
 */
- (BOOL)checkpoint:(UbiXDBCheckpointMode)checkpointMode error:(NSError * _Nullable *)error;
...
/** Performs a WAL checkpoint
 ...
 @return @c YES on success, otherwise @c NO .
 */
- (BOOL)checkpoint:(UbiXDBCheckpointMode)checkpointMode name:(NSString * _Nullable)name error:(NSError * _Nullable *)error;
...
/** Performs a WAL checkpoint
 ...
 @return @c YES on success, otherwise @c NO .
 */
- (BOOL)checkpoint:(UbiXDBCheckpointMode)checkpointMode name:(NSString * _Nullable)name logFrameCount:(int * _Nullable)logFrameCount checkpointCount:(int * _Nullable)checkpointCount error:(NSError * _Nullable *)error;
...
/// The UbiXDB version number as a string in the form of @c "2.7.8" .
///
/// If you want to compare version number strings, you can use NSNumericSearch option:
///
/// @code
/// NSComparisonResult result = [[UbiXDatabase UbiXDBUserVersion] compare:@"2.11.0" options:NSNumericSearch];
/// @endcode
///
/// @returns The version number string.

+ (NSString*)UbiXDBUserVersion;

/** The UbiXDB version

 This returns the UbiXDB as hexadecimal value, e.g., @c 0x0243 for version 2.4.3.
 ...
 @warning This routine will not work if any component of the version number exceeds 15.
       ...
       For this reason, we would recommend using @c UbiXDBUserVersion  and with @c NSNumericSearch  option, e.g.
...
+ (SInt32)UbiXDBVersion __deprecated_msg("Use UbiXDBUserVersion instead");
...
UbiXResultSet *rs = [db executeQuery:@"SELECT * FROM employees WHERE RemoveDiacritics(first_name) LIKE 'jose'"];
...
@code
myDB.dateFormat = [UbiXDatabase storeableDateFormat:@"yyyy-MM-dd HH:mm:ss"];
@endcode
 ...
 @warning Note that @c NSDateFormatter  is not thread-safe, so the formatter generated by this method should be assigned to only one UbiXDB instance and should not be used for other purposes.
...
/** Set to a date formatter to use string dates with sqlite instead of the default UNIX timestamps.
 ...
 @param format Set to nil to use UNIX timestamps. Defaults to nil. Should be set using a formatter generated using @c UbiXDatabase:storeableDateFormat .
 ...
 @warning Note there is no direct getter for the @c NSDateFormatter , and you should not use the formatter you pass to UbiXDB for other purposes, as @c NSDateFormatter  is not thread-safe.
 */
...
/** Objective-C wrapper for @c sqlite3_stmt
 ...
 This is a wrapper for a SQLite @c sqlite3_stmt . Generally when using UbiXDB you will not need to interact directly with `UbiXStatement`, but rather with @c UbiXDatabase  and @c UbiXResultSet  only.
...
@interface UbiXStatement : NSObject {
    void *_statement;
    NSString *_query;
    long _useCount;
    BOOL _inUse;
